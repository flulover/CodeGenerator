...
namespace Xsd2Code.Library.Extensions
{
    ...
    public class Net30Extension : CodeExtension
    {
        ...
        protected override void ProcessClass(CodeNamespace codeNamespace, XmlSchema schema, CodeTypeDeclaration type)
        {
            ...
            this.GenerateAutomaticProperties(codeNamespace, type);
        }
        ...
        protected override void CreateDataContractAttribute(CodeTypeDeclaration type, XmlSchema schema)
        {
            ...
            {
                var attributeType = new CodeTypeReference("DataContract");
...
//                            }
        ...
        /// <summary>
        /// Remove the redundancy namespace prefix.
        /// </summary>
        /// <param name="codeNamespace"></param>
        /// <param name="attributeFullName"></param>
        /// <returns></returns>
        private string GetShortestAttributeName(CodeNamespace codeNamespace, string attributeFullName)
        {
            var namespaceList = (from CodeNamespaceImport import in codeNamespace.Imports select import.Namespace).ToList();
            namespaceList.Sort((left, right) => right.Length - left.Length);

            foreach (string importNamespace in namespaceList)
            {
                if (attributeFullName.StartsWith(importNamespace))
                {
                    return attributeFullName.Substring(importNamespace.Length + 1);
                }
            }
        ...
        /// </summary>
        /// <param name="type">Represents a type declaration for a class, structure, interface, or enumeration.</param>
        private void GenerateAutomaticProperties(CodeNamespace codeNamespace, CodeTypeDeclaration type)
        {
