        ...
        private Result Process(string outputFilePath)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        using (var outputStream = new StreamWriter(type.Name + ".tmp", false))
                        {
                            ...
                            this.providerField.GenerateCodeFromNamespace(tmpNs, outputStream, codeGeneratorOptions);
                        }
                    }
                }
                catch (Exception e)
...
//                                commentStr);
                        ...
                        using (TextReader streamReader = new StreamReader(type.Name + ".tmp"))
                        {
                            string line;
                ...
                try
                {
                    foreach (CodeTypeDeclaration type in ns.Types)
                    {
                        var tempFile = new FileInfo(type.Name + ".tmp");
                        tempFile.Delete();
                    }
                }
                catch (Exception ex)
